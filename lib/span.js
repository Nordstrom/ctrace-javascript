...
class Span { // extends opentracing.Span {
  ...
  addTags (keyValueMap) {
    ...
    for (let key in keyValueMap) {
      if (keyValueMap.hasOwnProperty(key)) tags[key] = keyValueMap[key]
    }
  ...
  log (keyValuePairs, timestamp) {
    if (!keyValuePairs.timestamp) keyValuePairs.timestamp = timestamp || Date.now()
    this._fields.log = keyValuePairs
